...
Promise.all([
  ...
  const honras_tot = honras_agg
    .filter(d => d.data_mes.substr(5,2) === "12" || d.data_mes === "2020-08-01") // inclui data na mão para abril
    .map(d => ({"ano"   : d.data_mes.substr(0,4) === "2020" ? "2020(ago)" : d.data_mes.substr(0,4),
                "valor" : d.valor_acum}));
  ...
  const pontos_totais_anos = $svg_honras
    ...
    .append("circle")
      ...
      .attr("fill", "firebrick")
      .attr("r", 4)
      .attr("cx", d => x(new Date(d.categoria == "2020(ago)" ? "2020-08-01" : d.categoria + "-12-01")))
  ...
  const labels_totais_anos = $container_honras
    ...
    .append("p")
      ...
      .style("right", d => (w_honras - x(new Date(d.categoria == "2020(ago)" ? "2020-08-01" : d.categoria + "-12-01")) + 7) + "px")
